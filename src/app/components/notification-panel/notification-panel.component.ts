...
import { AppDataState, DataStateEnum, Employee, Notification, NotificationResponseDTO, TimesheetDTO } from 'src/app/models/timesheet.model';
...
export class NotificationPanelComponent implements OnInit {
  ...
  seeTimesheet(period:string, sender:Employee, idTobeTransferred:string){
    ...
    this.timesheetService.sEid=sender.id;
    
    if(sender.id==this.notificationResponseDTO.receiver.supervisorID){
      this.timesheetService.uEid=this.notificationResponseDTO.receiver.id;
      this.timesheetService.supervised=true;      
    }else {
      this.timesheetService.uEid=sender.id;
